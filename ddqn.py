...
def update_action(online_model, target_model, obs_batch):
  shuffle(obs_batch)
  batch_observations = []
  batch_targets = []
  ...
  for sample in obs_batch:
    s, a, r, s_ = sample
    q_est = model_inference(online_model, s)
    q_est = np.reshape(q_est, num_actions)
    q_est[a] = r
    if s_ is not None:
      pred = model_inference(target_model, s_)
      pred2 = model_inference(online_model, s_)
      a_ = np.argmax(pred)
      a2_ = np.argmax(pred2)
      q_est[a] += gamma * pred[0, a_]

      online_q.append(pred2[0,a2_])
      target_q.append(pred[0,a_])

    batch_observations.append(s)
    batch_targets.append(q_est)

  if np.random.random() < 0.5:
    train(online_model, batch_observations, batch_targets)
  else:
    train(target_model, batch_observations, batch_targets)


deq = deque()
online_model = model_init()
target_model = model_init()
...
for i in range(NUM_EPISODES):
  ...
  while(not done and reward < 500):
    ...
    if np.random.random() > epsilon:
      action = np.argmax(model_inference(online_model,current_state))
    else:
      action = np.random.randint(0,num_actions)
    ...
    if len(deq) >= minibatch and np.random.random()<0.3 and Samples[-1]<500:
      ...
      update_action(online_model, target_model, sample_history)
...
file = open('online_q.txt','w')
for item in online_q:
    file.write(f"{item}\n")
  #file.write(item+"\n")
file.close()

file = open('target_q.txt','w')
...
plt.ylim(ymax=510)
plt.xlabel('Trial')
plt.savefig('Training_ddqn.png', bbox_inches='tight')
plt.show()